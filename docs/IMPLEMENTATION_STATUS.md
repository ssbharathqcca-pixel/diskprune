Last verified product commit: `9d1259fe` (production k1 + D1/KV ids). Phase 7.5 (B-13 success page) is in this tree. Gate 4 remains **PASS** on `2241cbe5`. Gate 3 is **not** PASS — a live Stripe purchase and Mac activation are still required.
...
- [x] CI job 10 added (website success-page / T-WEB-05)
...
- [x] `PublicKeys.k1Base64` equals Worker `[vars] LICENSE_SIGNING_PUB_K1` (CI job 9)
- [x] macOS CI job 8 executes named native licensing tests (pipefail; required-name evidence)
- [x] Success page is status-only (B-13): `GET /v1/checkout/:id/status`; no client-side key generation; `issueLicense()` removed
- [ ] Live Stripe purchase + Mac activation (owner; remaining Gate 3 bar)

**Gate 3: NOT PASS** — infrastructure is deployed. A live purchase and Activate on a Mac app built from the matching public k1 are still required. Phase 7.1–7.4 / 7.6–7.8 (full Astro port, demo, SEO, delete `site/`) are not done.
